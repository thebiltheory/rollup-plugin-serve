...
function serve(options = { contentBase: "" }) {
  ...
  const httpS = options.https ? https : http;
  ...
  // Identifier 'http_s' is not in camel case.
  // Unnecessary escape character: \/.

  // 'statusCode' is assigned a value but never used.
  ...
  const requestListener = (request, response) => {
    ...
    const proxy = proxies.find(({ test }) => {
      request.url.match(test);
    });
    ...
    if (proxy && proxy.destination) {
      ...
      request.on("end", () => {
          ...
          .request(newDestination, { headers, method }, (proxyResponse) => {
            ...
            proxyResponse.on("end", () => {
              Object.keys(proxyResponse.headers).forEach((key) =>
                response.setHeader(key, proxyResponse.headers[key])
              );
        ...
        proxyRequest.on("error", (err) =>
          console.error(
            `There was a problem with the request for ${request.url}: ${err}`
          )
        );
        proxyRequest.end();
      });
